...
|------|----------|-------|-----------|
| User settings | `~/.opencode/settings.json` | All projects | No |
| Project settings | `.opencode/settings.json` | Current project | Yes |
| Local settings | `.opencode/settings.local.json` | Current project | No |
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".opencode/hooks/validate-bash.sh",
            "timeout": 30
          }
        ]
      }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.opencode/hooks/format-code.sh"
          }
        ]
      }
    ],
...
set -euo pipefail
cmd=$(jq -r '.tool_input.command // ""')
printf '%s %s\n' "$(date -Is)" "$cmd" >> .opencode/bash-commands.log
exit 0
```
...
<bad_example>
```bash
log_file="/home/user/project/.opencode/commands.log"
```
...
<good_example>
```bash
log_file="$CLAUDE_PROJECT_DIR/.opencode/commands.log"
```
