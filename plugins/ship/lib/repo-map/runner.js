  ...
  for (const [lang, files] of Object.entries(configIndicators)) {
    for (const file of files) {
      if (fs.existsSync(path.join(basePath, file))) {
        if (lang === 'java') {
          const hasNonTest = hasNonTestExtension(basePath, LANGUAGE_EXTENSIONS.java);
          if (hasNonTest) detected.add(lang);
        } else {
          detected.add(lang);
        }
        break;
  ...
  for (const lang of languages) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        referenceStateByFile.set(relativePath, createReferenceState());
    ...
    for (const [sgLang, entries] of filesBySgLang) {
      ...
      const patternGroups = [
        ...
        { category: 'references', patterns: langQueries.references, defaultKind: 'reference' }
      ];
        ...
        for (const patternDef of group.patterns) {
          ...
          for (const chunk of chunks) {
            ...
            for (const match of matches) {
              ...
              if (group.category === 'references') {
                const names = extractNamesFromMatch(match, patternDef);
                const referenceState = referenceStateByFile.get(matchedPath);
                if (!referenceState) continue;
                for (const name of names) {
                  addReferenceToState(referenceState, name, match, patternDef.kind || group.defaultKind);
                }
                continue;
              }
    ...
    for (const entry of fileEntries) {
      ...
      const referenceState = referenceStateByFile.get(relativePath);
      if (!symbolMaps || !importState) continue;
      ...
      if (referenceState) {
        map.files[relativePath].references = referenceMapToArray(referenceState);
      }
...
function addReferenceToState(state, name, match, kind) {
  ...
  if (state.count >= MAX_REFERENCES_PER_FILE) return;
...
/**
 * Extract references from a file using ast-grep
 * @param {string} cmd - ast-grep command
 ...
 */
function extractReferences(cmd, file, language, langQueries, basePath) {
  const references = [];

  if (!langQueries.references) return references;

  const sgLang = queries.getSgLanguageForFile(file, language);
  const seen = new Map();

  for (const patternDef of langQueries.references) {
    const pattern = patternDef.pattern || patternDef;
    const results = runAstGrep(cmd, file, pattern, sgLang, basePath);
    for (const match of results) {
      const names = extractNamesFromMatch(match, patternDef);
      for (const name of names) {
        ...
        seen.set(name, {
          name,
          line: getLine(match),
          kind: patternDef.kind || 'reference',
          count: 1
        });
      }
    }
...
function scanSingleFile(cmd, file, basePath) {
  ...
  try {
    ...
    const references = extractReferences(cmd, file, language, langQueries, basePath);
...
function referenceMapToArray(state) {
  if (!state || !state.items) return [];
  return Array.from(state.items.values()).sort((a, b) => a.name.localeCompare(b.name));
}
