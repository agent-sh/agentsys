...
function extractFeaturesFromContent(content, filePath, options) {
  ...
  const usageFeatures = extractCliUsageFeatures(rawLines, filePath, options);
  for (const record of usageFeatures) {
    if (features.length >= options.maxPerFile) break;
    features.push(record);
  }
...
function extractInlineFeature(line) {
  ...
  const featuringMatch = line.match(/\bfeaturing\s+(.+)/i);
  if (featuringMatch) {
    let candidate = cleanupFeatureText(featuringMatch[1]);
    candidate = candidate.replace(/^(?:an?|the)\s+/i, '');
    const list = splitConjunctionFeatures(candidate);
    if (list) return list;
    if (candidate.length < 10) return null;
...
function extractOptionFeature(rawText, lines, index) {
  ...
  if (!text) return null;
    ...
    if (!flag || flag === 'h' || flag === 'help' || flag === 'version') return null;
...
function extractCliUsageFeatures(rawLines, filePath, options) {
  const features = [];
  ...
  const flushBlock = () => {
    ...
    const blockLines = block.map(item => item.line);
    if (!blockLines.some(line => /^\s*usage(?: of)?\b/i.test(String(line || '')))) {
      block = [];
      return;
    }
    for (let i = 0; i < blockLines.length; i += 1) {
      const line = String(blockLines[i] || '');
      if (!/^\s*-{1,2}[a-z0-9][\w-]*/i.test(line)) continue;
      let candidate = extractOptionFeature(line, blockLines, i);
      if (!candidate) continue;
      const desc = findNextNonEmptyLine(blockLines, i + 1);
      if (desc) {
        const cleaned = cleanupFeatureText(desc);
        if (cleaned && !isLowSignalText(normalizeText(cleaned))) {
          candidate = cleaned;
        }
      }
      candidate = stripLeadingVerb(candidate);
      const record = buildFeatureRecord(candidate, filePath, block[i].index + 1, line, options);
      if (record) {
        features.push(record);
        if (features.length >= options.maxPerFile) break;
      }
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const trimmedLine = String(line || '').trim();
    if (inFence) {
      if (isClosingFence(trimmedLine, fence)) {
        flushBlock();
        inFence = false;
        fence = null;
        continue;
      }
      block.push({ line, index: i });
      continue;
    }
    if (isMarkdownFenceLine(trimmedLine) || isAdocFenceLine(trimmedLine)) {
      inFence = true;
      fence = trimmedLine;
      block = [];
      continue;
...
function buildFeatureRecord(name, filePath, lineNumber, contextLine, options) {
  ...
  if (sourceType !== 'release' && tokens.length < 2) {
    ...
    const allowInlineList = contextLine && /\b(things like|such as|including|featuring)\b/i.test(contextLine);
...
function splitConjunctionFeatures(text) {
  const raw = String(text || '').trim();
  if (!raw) return null;
  if (!raw.includes(',') && !/\band\b/i.test(raw)) return null;
