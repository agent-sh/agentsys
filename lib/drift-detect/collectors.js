...
function shouldSkipCheckboxPath(filePath) {
  ...
  if (normalized.endsWith('docs/testing.md') || normalized.endsWith('docs/test.md')) return true;
  if (/(^|\/)checklists\//.test(normalized)) return true;
  if (/(^|\/)agent-docs\//.test(normalized)) return true;
  if (/(^|\/)references\//.test(normalized)) return true;
  if (/(^|\/)research\//.test(normalized)) return true;
  if (/(^|\/)docs\/reference\//.test(normalized)) return true;
  if (/(^|\/)docs\/references\//.test(normalized)) return true;
  if (/(^|\/)docs\/internal\//.test(normalized)) return true;
  if (/(^|\/)docs\/specs\//.test(normalized)) return true;
  if (/(^|\/)docs\/drafts\//.test(normalized)) return true;
  if (/(^|\/)docs\/guides?\//.test(normalized)) return true;
  return false;
}
...
function buildDriftSummary(docs, code, opts) {
  ...
  summary.planEvidence = buildPlanEvidence(docs, code, opts);
  return summary;
}
...
function buildPlanEvidence(docs, code, opts) {
  const repoMap = code?.repoMap;
  if (!repoMap?.available) {
    return { available: false, total: 0, items: [] };
  }

  const items = collectPlanItems(docs);
  if (items.length === 0) {
    return { available: true, total: 0, items: [] };
  }

  const termLimit = Math.min((opts?.repoMap?.maxFeatures || 30) * 2, items.length * 2);
  const evidenceTerms = buildEvidenceTerms(items, termLimit);
  const evidence = getRepoMap().findFeatureEvidence(opts.cwd, evidenceTerms, {
    maxFeatures: termLimit,
    maxDefsPerFeature: opts.repoMap?.maxDefsPerFeature,
    maxRefsPerFeature: opts.repoMap?.maxRefsPerFeature,
    maxFilesScannedPerDef: opts.repoMap?.maxFilesScannedPerDef,
    snippetLines: opts.repoMap?.snippetLines
  ...
  const evidenceMap = buildEvidenceMap(evidence?.features);
  const output = items.map(item => {
    const evidenceItem = resolveEvidenceForText(item.text, evidenceMap);
    const codeEvidence = evidenceItem
      ? summarizeCodeEvidence(evidenceItem, opts?.repoMap)
      : { status: 'missing', defs: [], refs: [], snippets: [] };
...
function collectPlanItems(docs) {
  const items = [];
  const seen = new Set();

  const checkboxItems = Array.isArray(docs?.checkboxes?.items) ? docs.checkboxes.items : [];
  for (const item of checkboxItems) {
    ...
    items.push({
      ...
      line: null,
      context: null,
      sources: item.file ? [item.file] : []
    });
  }

  const planItems = Array.isArray(docs?.plans) ? docs.plans : [];
  for (const text of planItems) {
    if (!text) continue;
  ...
  for (const feature of detailFeatures) {
    if (!feature || (!feature.plan && feature.sourceType !== 'plan')) continue;
    ...
    items.push({
      ...
      line: feature.sourceLine || null,
      context: feature.context || null,
      sources: Array.isArray(feature.sources)
        ? feature.sources.map(source => source.file).filter(Boolean)
        : (feature.sourceFile ? [feature.sourceFile] : [])
    });
  }
...
function buildEvidenceMap(features) {
  const map = new Map();
...
function buildEvidenceTerms(items, limit) {
  const terms = [];
  const seen = new Set();
  for (const item of items) {
  ...
  for (const item of items) {
    if (terms.length >= limit) break;
    const aliases = getPlanEvidenceAliases(item.text);
...
function resolveEvidenceForText(text, evidenceMap) {
  const normalized = featureExtractor.normalizeText(text);
  if (!normalized) return null;
  if (evidenceMap.has(normalized)) return evidenceMap.get(normalized);
  const aliases = getPlanEvidenceAliases(text);
  for (const alias of aliases) {
    const aliasNorm = featureExtractor.normalizeText(alias);
    if (!aliasNorm) continue;
    const match = evidenceMap.get(aliasNorm);
    if (match) return match;
  }
  return null;
}

function getPlanEvidenceAliases(text) {
  const normalized = featureExtractor.normalizeText(text);
  if (!normalized) return [];

  const rules = [
    { match: /ast queries per language/, terms: ['ast-grep', 'repo map runner', 'repo-map runner'] },
    { match: /cache management.*state[- ]dir/, terms: ['state-dir', 'source-cache', 'repo-map cache'] },
    { match: /ship post-merge update/, terms: ['ship', 'post-merge', 'ship workflow'] },
    { match: /drift-detect suggestion/, terms: ['drift-detect', 'repo-map installer', 'install suggestion'] },
    { match: /agent awareness/, terms: ['agent analyzer', 'agent patterns', 'agent'] },
    { match: /data flow/, terms: ['collectors', 'plan-synthesizer', 'workflow'] }
  ];
...
function summarizePlanDrift(docs, code, opts) {
  ...
  const termLimit = Math.min(limit * 2, (opts.repoMap?.maxFeatures || 30) * 2);
  const evidenceTerms = buildEvidenceTerms(slice, termLimit);
  const evidence = getRepoMap().findFeatureEvidence(opts.cwd, evidenceTerms, {
    maxFeatures: termLimit,
    maxDefsPerFeature: opts.repoMap?.maxDefsPerFeature,
    maxRefsPerFeature: opts.repoMap?.maxRefsPerFeature,
