...
function getCompiledPattern(pattern) {
  if (!_compiledExcludePatterns.has(pattern)) {
    ...
    // Convert glob patterns to safe regex:
    // - ** (globstar) -> .* (matches anything including path separators)
    // - * (single star) -> [^/\\]* (matches anything except path separators - prevents backtracking)
    let regexStr = escaped
      .replace(/\*\*/g, '\0GLOBSTAR\0')  // Temporarily mark globstar
      .replace(/\*/g, '[^/\\\\]*')        // Single star: non-greedy, no path separators
