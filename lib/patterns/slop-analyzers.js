  ...
  for (const pattern of config.docPatterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (ratio > maxRatio) {
        ...
        const funcName = match[2] || match[3] || match[4] || match[5] || 'unknown';
        violations.push({
          line: lineNumber,
          docLines: docLines,
          codeLines: funcLines,
...
function analyzeDeadCode(content, options = {}) {
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    // Check for multi-line statements (unbalanced parentheses)
    // e.g., "throw new Error(" continues on next lines
    let parenBalance = (trimmed.match(/\(/g) || []).length - (trimmed.match(/\)/g) || []).length;
    if (parenBalance > 0) {
      // Statement continues on following lines - skip to where it ends
      for (let k = i + 1; k < lineCount && parenBalance > 0; k++) {
        const contLine = lines[k].trim();
        parenBalance += (contLine.match(/\(/g) || []).length - (contLine.match(/\)/g) || []).length;
  ...
  const configs = {
    js: {
      ...
      functionPatterns: [
        ...
        /^\s*(?:async\s+)?(?!if\b|for\b|while\b|switch\b|catch\b|with\b)(\w+)\s*\([^)]*\)\s*\{/gm,
      ],
    ...
    go: {
      ...
      stubReturnPattern: /^\s*return\s+(nil|0|""|false|true|\[\]\w*\{\}|map\[\w+\]\w*\{\}|struct\{\}\{\})\s*$/,
