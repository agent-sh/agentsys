 ...
 *
 * Strips existing frontmatter, prepends new YAML frontmatter with
 * inclusion: manual, replaces PLUGIN_ROOT paths, removes require()
 * statements and Task() calls, strips namespace prefixes.
 *
 * @param {string} content - Source command markdown content
 * @param {Object} options
 * @param {string} options.pluginInstallPath - Absolute path to plugin install dir
 * @param {string} options.name - Steering file name
 * @param {string} options.description - Steering file description
 * @returns {string} Transformed steering file content
 */
function transformCommandForKiro(content, options) {
  ...
  // Build Kiro steering frontmatter
  const cleanDescription = description.replace(/[\x00-\x1f\x7f]/g, ' ');
  const escapedDescription = cleanDescription.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  let frontmatter = '---\n';
 ...
 *
 * Parses the markdown frontmatter for name, description, model, and tools.
 * Uses the body (after frontmatter) as the prompt. Returns a JSON string.
 *
 * @param {string} content - Source agent markdown content with frontmatter
 * @param {Object} [options]
 * @returns {string} JSON string of Kiro agent definition
 */
function transformAgentForKiro(content, options) {
  ...
  // Strip plugin namespacing from body
  ...
  if (frontmatter.tools) {
    const toolStr = frontmatter.tools.toLowerCase();
    const tools = [];
    ...
    agent.tools = ['read', 'write', 'shell'];
  }

  // Add resources (steering files available to the agent)
