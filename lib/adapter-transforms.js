 ...
 * @param {string} options.ruleName - Rule name (e.g. "agentsys-next-task")
 ...
 */
function transformForCursor(content, options) {
  const { ruleName, description, pluginInstallPath, globs = '', alwaysApply = true } = options;
  ...
  // Strip Claude-specific syntax: Task tool calls
  content = content.replace(/await\s+Task\s*\(\s*\{[\s\S]*?\}\s*\);?/g, (match) => {
    const agentMatch = match.match(/subagent_type:\s*["'](?:[^"':]+:)?([^"']+)["']/);
    if (agentMatch) {
