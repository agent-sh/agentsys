 ...
 * Returns a fresh default object when file is absent or corrupted.
 */
 ...
 *
 * Pattern (mirrors updateFlow for flow.json):
 *   1. Read current state, remember _version
 *   2. Apply mutatorFn(clone) → new state
 *   3. Write atomically (increments _version)
 *   4. Re-read: if _version === initialVersion + 1 → we won, done
 *   5. Otherwise another writer raced us → back off, retry
 *
 ...
 * @returns {boolean} true on success, false after MAX_RETRIES exhausted
 */
function updateTasks(mutatorFn, projectPath = process.cwd()) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    // Another writer landed between our read and write — retry after jitter
  ...
  // All retries exhausted — read one final time to see if a concurrent writer
  // happened to apply an equivalent mutation (idempotent operations).
  console.error(
