...
/**
 * Known project memory file names in order of preference
 * Supports both Claude Code and cross-platform alternatives
 */
...
function extractFileReferences(content) {
  ...
  const backtickMatches = content.match(/`([^`]+\/[^`]+\.[a-z]+)`/gi) || [];
...
function calculateTokenMetrics(content, readmeContent = null) {
  if (!content || typeof content !== 'string') {
    return { estimatedTokens: 0, characterCount: 0, lineCount: 0 };
  }
...
function analyzeFile(filePath, options = {}) {
  ...
  results.metrics = calculateTokenMetrics(content, readmeDuplication.hasReadme ? fs.readFileSync(readmeDuplication.readmePath, 'utf8') : null);

  // Create context for pattern checks
  const context = {
  ...
  // Run cross-platform pattern checks
...
function analyze(targetPath, options = {}) {
  ...
  if (fs.existsSync(targetPath)) {
    ...
    if (stat.isFile()) {
      ...
      // Directory - find project memory file
 ...
 * Currently no auto-fixes are available for project memory patterns
 * @param {Object} results - Analysis results
 * @param {Object} options - Fix options
 * @returns {Object} Fix results
 */
function applyFixes(results, options = {}) {
  // Currently no auto-fixes implemented
  // Project memory files require human judgment for fixes
